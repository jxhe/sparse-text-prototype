...
def collate(
    ...
    edit_vecs = torch.tensor([s['edit_vecs'] for s in samples])
    edit_vecs = edit_vecs.index_select(0, sort_order)
...
class LanguagePairEditDataset(LanguagePairDataset):
    """A slight addon to LanguagePairDataset so that
    each sample in the batch has a edit vector as input
    """
    def __init__(
        ...
        align_dataset=None,
        append_bos=False, eos=None, edit_vecs=None
    ):
        super(LanguagePairEditDataset, self).__init__(
        ...
        self.edit_vecs = edit_vecs

    def __getitem__(self, index):
        sample = super(LanguagePairEditDataset, self).__getitem__(index)
        sample.update({'edit_vecs': self.edit_vecs[index]})
