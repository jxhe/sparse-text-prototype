"""
The code is upon some modification based on 
...
class vMF(torch.nn.Module):
    def __init__(self, hid_dim, lat_dim, kappa=1, cuda=True):
        """
        ...
        self.kld = torch.from_numpy(vMF._vmf_kld(kappa, lat_dim)).float().to(self.device)
        self.log_norm = vMF._log_normalization_constant(kappa, lat_dim)

        # set to 1e-10 to avoid numeric error
        self.log_norm_uniform = vMF._log_normalization_constant(0, lat_dim)
    ...
    @staticmethod
    def _log_normalization_constant(k, d):
        # When k=0, we go to the limit case: C = gamma(d/2 + 1) / (d * pi*{d/2})
        # reference: https://en.wikipedia.org/wiki/N-sphere
        if k == 0:
            tmp = sp.loggamma(d / 2.0 + 1) - np.log(d) - d / 2.0 * np.log(np.pi)
            return tmp

        tmp = (d / 2.0 - 1) * np.log(k) - d / 2.0 * np.log(2 * np.pi) - np.log(sp.iv(d / 2.0 - 1.0, k))

        if tmp != tmp:
            exit()
        return tmp

    def log_density(self, k, x, mu=None):
        """compute log density under vmf distribution
        Args:
            mu: tensor with shape (batch, *, latent_dim)
            x: tensor with shape (batch, *, latent_dim)

        Returns:
            density: tensor with shape (batch, *)
            
        """

        if k == 0:
            return self.log_norm_uniform + k * (x.sum(dim=-1))

        return self.log_norm + k * (mu * x).sum(dim=-1)
    ...
    def sample_cell(self, mu, norm, kappa):
        ...
        # batch version
        w_var = (w * torch.ones(batch_sz, lat_dim)).to(self.device)
        v = self._sample_ortho_batch(mu, lat_dim)
        scale_factr = torch.sqrt(
            torch.ones(batch_sz, lat_dim, device=self.device) - torch.pow(w_var, 2))
        orth_term = v * scale_factr
        muscale = mu * w_var
        sampled_vec = orth_term + muscale

        return sampled_vec.unsqueeze(1)
    ...
    def _sample_ortho_batch(self, mu, dim):
        ...
        v = torch.randn(_batch_sz, dim, 1, device=self.device)  # TODO random
    ...
    def _sample_orthonormal_to(self, mu, dim):
        ...
        """
        v = torch.randn(dim, device=self.device)  # TODO random
