...
class SequenceGenerator(object):
    ...
    def _generate(
        ...
        **kwargs
        ...
        edit_vecs = encoder_input['edit_vecs']
        ...
        edit_vecs = edit_vecs.index_select(0, new_order)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                edit_vecs = edit_vecs.index_select(0, reorder_state)

            lprobs, avg_attn_scores = model.forward_decoder(
                tokens[:, :step + 1], encoder_outs, temperature=self.temperature, edit_vecs=edit_vecs,
            )
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, temperature=1., edit_vecs=None):
    ...
    def _decode_one(
        ...
        temperature=1., edit_vecs=None
    ):
        if self.incremental_states is not None:
            decoder_out = list(model.forward_decoder(
                tokens, encoder_out=encoder_out, incremental_state=self.incremental_states[model],
                edit_vector=edit_vecs,
            ))
        else:
            decoder_out = list(model.forward_decoder(tokens, encoder_out=encoder_out, edit_vecs=edit_vecs))
