...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    with torch.no_grad():
                        lambda_stats_sum_i = self.task.collect_lambda_stats(self.model, sample)
        ...
        try:
            ...
            with torch.no_grad():
                if self._sync_stats():
                    torch.distributed.all_reduce(lambda_stats_sum, op=torch.distributed.ReduceOp.SUM)
                    # print('nsentences {} per gpu'.format(nsentences))
                    nsentences_t = torch.tensor(nsentences, device=self.device)
                    torch.distributed.all_reduce(nsentences_t, op=torch.distributed.ReduceOp.SUM)
    ...
    def valid_step(self, sample, raise_oom=False, split='valid'):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.criterion, split=split
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
                    if not raise_oom:
                        ...
                        if self.cuda:
                            torch.cuda.empty_cache()
                        return self.valid_step(sample, raise_oom=True, split=split)
                raise e
    ...
    @metrics.aggregate("valid_iw")
    def valid_iw_step(self, sample, raise_oom=False, mode='iw'):
        """Do forward pass in evaluation mode."""
        if self._dummy_batch == "DUMMY":
            self._dummy_batch = sample
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_iw_step(
                    sample, self.model, self.criterion, mode=mode
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
                    if not raise_oom:
                        ...
                        if self.cuda:
                            torch.cuda.empty_cache()
                        return self.valid_iw_step(sample, raise_oom=True)
                raise e
